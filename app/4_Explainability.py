...
def main():
    ...
    if model_name == "EBM":
        ...
        if ebm_features is not None:
            st.caption(f"EBM feature names: {list(ebm_features[:5])} ... ({len(ebm_features)} total)")
        else:
            st.warning("EBM feature_names_in_ not set — charts may show generic indices. Re-train the model.")
    ...
    idx = st.number_input("Applicant row index", min_value=0, max_value=max(0, len(X) - 1), value=0, step=1)
    ...
    feature_for_pdp = st.selectbox("Feature", X.columns.tolist())
